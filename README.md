...
**A self-hostable artifact service** — REST API, MCP server, web UI, and a
CLI, backed by Postgres plus any S3-compatible object store. Humans browse,
view, and comment; agents and scripts upload and read via CLI, REST, or MCP.

- **Store versioned artifacts** — markdown / HTML / plaintext / multi-file
  `PACKAGE` bundles (and sandboxed `APP` mini-sites). A stable `slug` makes an
  artifact human-shareable; re-uploads to a slug auto-version.
- **Fetch by UUID or slug** — `/a/{uuid}`, `/s/{slug}`, `/s/{slug}/v_{N}`.
- **List, search, filter** — full-text search (optional OpenSearch, Postgres
  fallback), plus `scope`, `labels[]`, type, and creator filters.
- **One service layer, four surfaces** — every operation is available over
  REST, MCP, the CLI, and the web UI.
