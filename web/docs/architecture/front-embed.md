...
### Zero-click reloads — the embedding-host token relay

The minted token lives only in the iframe page's memory (an opaque-origin
sandbox has no storage), so without help every iframe load would repeat the
consent popup. An embedding host can opt in to a small relay contract so the
viewer consents once per token lifetime instead of once per load:

- **Page → host:** after every successful mint the bridge posts
  `{type: "arti-app:token", appId, token, exp}` to `window.parent`, with
  `targetOrigin` pinned to each entry of the surface's configured `origin`
  list — the same set `frame-ancestors` admits, so the token can only be
  delivered to a window the surface already declared as its embedder. `exp`
  is the JWT expiry (seconds), read client-side, so the host can expire its
  cache without decoding tokens.
- **Host → page:** the host caches the token (e.g. per-tab `sessionStorage`)
  and appends it to the iframe URL as a fragment — `#arti_token=<token>` —
  on later loads. Fragments never reach the server or its logs; the bridge
  adopts the token on boot and scrubs it from the URL.
- **Fallback:** a stale or revoked hand-in fails the proxy's checks (a plain
  `401`, or a `403 app_id` mismatch when the app was republished — tokens pin
  the exact artifact version) and the bridge falls back to the normal connect
  flow, re-announcing the fresh token once minted.

The relay never mints: every token still originates from the consent popup.
Hosts that ignore the contract keep today's behavior unchanged.
