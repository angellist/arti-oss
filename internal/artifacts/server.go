...
func (s *Service) httpApp(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.serveAppRow(w, r, row, caller, "", "", ""); err != nil {
		writeBadOrInternal(w, err)
	}
}
...
// to be "".
func (s *Service) serveAppRow(w http.ResponseWriter, r *http.Request, row sqlc.Artifact, caller, frameAncestors, filesBase, userSurface string) error {
	...
	if userSurface != "" {
		body = s.injectAppBridgeUser(body, ct, aid, userSurface, params)
	} else {
...
const appBridgeJS = `(function(){
  ...
  function connect(){
    ...
    pendingConnect = new Promise(function(resolve, reject){
      ...
      if(!pop){ pendingConnect = null; reject(new Error("popup blocked - use the Connect button")); return; }
      var done = false, deadline = Date.now() + 180000; // 3 min
      function finish(err, tok){
        if(done){ return; }
        done = true; pendingConnect = null;
        if(err){ reject(err); } else { hideConnectButton(); resolve(tok); }
      }
      function tick(){
        if(done){ return; }
        ...
        fetch(pollURL(state), { credentials: "omit" })
          ...
          .then(function(b){
            if(b && b.ready && b.token){ cfg.token = b.token; finish(null, b.token); return; }
  ...
  function run(server, tool, args){
    return rpc(server, tool, args).then(function(res){
      ...
      if(res.status === 401 && cfg.connect){
        // The OTHER 401: an expired/invalid app token (the proxy's plain
        // "invalid app token"), not an OBO consent ask. Re-mint rides this
        // same user click - silent once consent is cached - then retry once.
        cfg.token = null;
...
func (s *Service) injectAppBridgeUser(body []byte, ct, artifactID, surface string, params map[string]any) []byte {
	if !strings.HasPrefix(strings.ToLower(ct), "text/html") {
		return body
	}
	cfgM := map[string]any{
