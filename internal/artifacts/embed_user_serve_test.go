...
// the handshake needs (surface + mint path + the version-pinned appId).
func TestInjectAppBridgeUser_ConnectConfigNoToken(t *testing.T) {
	s := &Service{}
	out := string(s.injectAppBridgeUser([]byte("<html><body></body></html>"), "text/html", "AID-1", "panel", nil))
	...
	if got := string(s.injectAppBridgeUser([]byte("{}"), "application/json", "A", "panel", nil)); got != "{}" {
...
func TestAppBridgeJS_UserModeBranches(t *testing.T) {
	for _, want := range []string{
		...
		"res.status === 401 && cfg.connect", // second 401 → re-mint path
