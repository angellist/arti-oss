...
func serveFrontShell(w http.ResponseWriter, surface string, surf Surface) {
	...
	// Inject the surface name, shell_slug, and shell_query as JS string literals.
	// All are operator-configured (not user input); JSON-encode defensively anyway.
	page := frontShellHTML
	...
	page = strings.ReplaceAll(page, "__SHELL_QUERY__", jsString(surf.ShellQuery))
...
// frontShellHTML is the Front adapter page. __SURFACE__ / __SHELL_SLUG__ /
// __SHELL_QUERY__ are replaced with JS string literals at serve time. It frames a
// nested iframe and swaps its src as the user moves between conversations, keeping
// the SDK subscription alive (a top-level redirect would lose it).
...
<script>
(function(){
  var SURFACE = __SURFACE__, SHELL_SLUG = __SHELL_SLUG__, SHELL_QUERY = __SHELL_QUERY__;
  ...
  function docURL(slug, q){
    return "/embed/" + SURFACE + "?slug=" + encodeURIComponent(slug) +
           "&auth_secret=" + encodeURIComponent(secret) +
           (q ? "&" + q : "") + (nonce ? "&_r=" + nonce : "");
  }
  function show(cnv){
  ...
  function none(text){ current = ""; curQ = ""; frame.style.display = "none"; rf.style.display = "none"; msg.style.display = "block"; msg.textContent = text; }
  rf.addEventListener("click", function(){ if(current){ nonce++; frame.src = docURL(current, curQ); } });
